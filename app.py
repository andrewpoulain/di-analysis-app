...
if st.session_state.get('analysis_complete'):
    ...
    fig_main.update_layout(
        title=(
            ...
            + " — Measured Response and Predicted "
            + "Room Curve (1/3 octave bands)"),
        xaxis=dict(
            title='Frequency (Hz)',
            type='log',
            tickvals=[
    ...
    if formula_rows:
        with st.expander(
            ...
            st.dataframe(
                ...
                hide_index=True)
            st.caption(
                "Sabine used when mean absorption "
                "alpha <= 0.2. "
                "Eyring used when alpha > 0.2. "
                "Eyring is more accurate for treated "
                "rooms with high absorption.")
    ...
    st.caption(
        ...
        "(max of 50 ms floor and mixing time estimate "
        "for V = " + str(round(volume)) + " m3).")
    ...
    st.caption(
        ...
        "Late energy window: "
        + str(round(late_start_val, 1))
        + " ms (max of 50 ms floor and mixing time "
        "estimate for V = "
        + str(round(volume)) + " m3).")
    ...
    st.caption(
        ...
        "All curves are anchored to the measured direct "
        "field level at 1 kHz.")
        ...
        with col1:
            st.download_button(
                ...
                help=(
                    ...
                    "applied. Use as a reference curve "
                    "in Smaart."))
